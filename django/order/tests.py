...
테스트 범위:
  ...
  - OrderService.handle_serving_rollback_event
...
# ═════════════════════════════════════════════
# OrderService.handle_serving_rollback_event 테스트
# ═════════════════════════════════════════════

@override_settings(CHANNEL_LAYERS=CHANNEL_LAYERS_TEST)
class HandleServingRollbackEventTest(TransactionTestCase, OrderTestMixin):
    ...
    def test_serving_to_cooked_rollback(self):
        """SERVING → COOKED 롤백 성공"""
        order, [item] = self._create_order_with_items(
            self.table_usage,
            [{"menu": self.menu, "quantity": 1, "fixed_price": 5000, "status": "SERVING"}],
        )

        result = OrderService.handle_serving_rollback_event(
    ...
    def test_rollback_not_serving_status(self):
        """SERVING이 아닌 상태에서 롤백 시도 → invalid_status"""
        order, [item] = self._create_order_with_items(
            self.table_usage,
            [{"menu": self.menu, "quantity": 1, "fixed_price": 5000, "status": "COOKED"}],
        )
        ...
        assert result["result"] == "invalid_status"
        assert result["current_status"] == "COOKED"

        item.refresh_from_db()
        assert item.status == "COOKED"

    def test_rollback_cooking_status(self):
        """COOKING 상태에서 롤백 시도 → invalid_status"""
        order, [item] = self._create_order_with_items(
            self.table_usage,
            [{"menu": self.menu, "quantity": 1, "fixed_price": 5000, "status": "COOKING"}],
        )
    ...
    def test_rollback_not_found(self):
        """존재하지 않는 아이템 → not_found"""
        result = OrderService.handle_serving_rollback_event(
            {"order_item_id": 99999}
        )
        assert result["result"] == "not_found"

    def test_rollback_missing_order_item_id(self):
        """order_item_id 누락 → missing_order_item_id"""
        result = OrderService.handle_serving_rollback_event({})
        assert result["result"] == "missing_order_item_id"

    def test_rollback_setmenu_parent_blocked(self):
        """세트메뉴 부모 아이템 롤백 → invalid_target"""
        setmenu = self._create_set_menu(self.booth)
        order, [parent] = self._create_order_with_items(
            self.table_usage,
            [{"setmenu": setmenu, "quantity": 1, "fixed_price": 12000, "status": "SERVING"}],
        )

        result = OrderService.handle_serving_rollback_event(
            {"order_item_id": parent.pk}
        )

        assert result["result"] == "invalid_target"

    def test_rollback_setmenu_child(self):
        """세트메뉴 자식 아이템 SERVING → COOKED 롤백 성공"""
        setmenu = self._create_set_menu(self.booth)
        order, [parent, child] = self._create_order_with_items(
            self.table_usage,
            [
                {"setmenu": setmenu, "quantity": 1, "fixed_price": 12000, "status": "SERVING"},
                {"menu": self.menu, "quantity": 1, "fixed_price": 0, "status": "SERVING", "parent": None},
            ],
        )
        # parent 연결
        child.parent = parent
